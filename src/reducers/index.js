...
function appData(state = { jewels: [], selectedJewel: [], score: 0, time: 0, animationPhase: NEUTRAL }, action) {
  switch (action.type) {
    ...
    case CHECK_FOR_SEQUENCES:
      ...
      //console.log(seqs)
      return { ...state, jewels: [...highlightJewels, ...normalJewels], animationPhase: highlightJewels.length > 0 ? HIGHLIGHT_SEQUENCES : NEUTRAL };
    case SEQUENCE_FOUND:
      return { ...state, jewels: _animateCollapse(state.jewels), animationPhase: COLLAPSE_SEQUENCE }
    case COLLAPSE_COMPLETE:
      return { ...state, jewels: _removeDuplicates(state.jewels), animationPhase: REMOVE_DUPLICATES }
    case EXIT_ANIMATION:
      return { ...state, jewels: _animateRemoval(state.jewels), animationPhase: REMOVE_EXITED }
    case REMOVE_EXITED:
      return { ...state, jewels: _removeShrunks(state.jewels), animationPhase: APPLY_GRAVITY }
    case APPLY_GRAVITY:
      return { ...state, jewels: _applyGravity(state.jewels), animationPhase: REPLACE_MISSING }
    case REPLACE_MISSING:
      return { ...state, jewels: _replaceMissingJewels(state.jewels, action.onClickHandler), animationPhase: INTRO_ANIMATION }
    case JEWEL_CLICK:
      return { ...state, selectedJewel: action.jewel, animationPhase: JEWEL_CLICK_IN }
    case SELECT_JEWEL:
      const selectedJewel = state.jewels.filter(j => j.row === action.row && j.column === action.column).map(j => ({ ...j, isSelected: true })),
        otherJewels = state.jewels.filter(j => j.row !== action.row || j.column !== action.column).map(j => ({ ...j, isSelected: false }));
      return { ...state, selectedJewel: [action.row, action.column], jewels: [...selectedJewel, ...otherJewels] }
    case SWAP_JEWELS:
      return { ...state, jewels: _swapJewels(action.jewels[0], action.jewels[1], state.jewels),  animationPhase: COMPLETE_SWAP }
    case COMPLETE_SWAP:
        return {...state, jewels: state.jewels.map(j=>({...j, animate: {direction: "static"}})), animationPhase: NEUTRAL}
    default:
      return state;
  }
}
